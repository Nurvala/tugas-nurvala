...
for ($i = 0; $i <count($datasekolah->SD->Jakarta->Kebayoran_lama);$i++){
	...
	$table .= "<tr>
				...
				<td>{$datasekolah->SD->Jakarta->Kebayoran_lama[$i]->provinsi}</td>
				<td>{$datasekolah->SD->Jakarta->Kebayoran_lama[$i]->status}</td>
				<td>{$datasekolah->SD->Jakarta->Kebayoran_lama[$i]->jenjang}</td>
				</tr>";
}
